...
function DelayMNRMJumpAggregation(
    nj::Int,
    njt::T,
    et::T,
    crs::Vector{T},
    ...
    if dep_graph === nothing
        if (get_num_majumps(maj) == 0) || !isempty(rs)
            error(
                "To use ConstantRateJumps with the Delay Modified Next Reaction Method (Delay MNRM) algorithm a dependency graph must be supplied.",
            )
        else
    ...
    return DelayMNRMJumpAggregation{T,S,F1,F2,RNG,typeof(dg),typeof(pq)}(
        nj,
        nj,
        njt,
        et,
...
function update_dependent_rates_delay!(
    p::DelayMNRMJumpAggregation, integrator, u, params, t
)
    ...
    @inbounds for rx in dep_rxs
        ...
        @inbounds cur_rates[rx] = calculate_jump_rate(
            ma_jumps, num_majumps, rates, u, params, t, rx
        )
        ...
        if rx != p.next_jump && oldrate > zero(oldrate)
            if cur_rates[rx] > zero(eltype(cur_rates))
                DataStructures.update!(
                    p.pq, rx, t + oldrate / cur_rates[rx] * (p.pq[rx] - t)
                )
            else
