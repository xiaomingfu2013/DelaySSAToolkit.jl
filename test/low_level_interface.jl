...
affect! = function (integrator)
    # integrator.u[3] -= 1
    return integrator.u = setindex(integrator.u, integrator.u[3] - 1, 3)
end
rate2 = (u, p, t) -> 0.1 * u[3]
...
for alg in algs
    djprob__ = DelayJumpProblem(
        ...
        dep_graph=dep_gr,
        dep_graph_delay=dep_gr_delay,
    )
    @info "Testing method $(alg)"
    sol = solve(djprob__, SSAStepper(); seed=1)

    djprob_no_dep_gr_delay = DelayJumpProblem(
        dprob_,
        alg,
        jumpset_,
        delayjumpset,
