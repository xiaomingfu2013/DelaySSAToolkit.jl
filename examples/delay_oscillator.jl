...
using Plots
plot(sol.t,[sol.u[i][1] for i in eachindex(sol.u)],alpha=0.3, label="X",color="green",linewidth = 2, legend = :top, ylabel = "# of individuals", xlabel = "Time", fmt=:svg)

sol = solve(djprob, SSAStepper(), seed = 1234)
plot!(sol.t,[sol.u[i][1] for i in eachindex(sol.u)], label="X",linewidth = 2, line=:dash,color="green")
